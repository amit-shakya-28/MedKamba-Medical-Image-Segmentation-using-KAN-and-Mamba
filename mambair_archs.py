...
# from omnimamba import SelectiveScanCore,SelectiveScanOflex,CrossMerge_Ab_1direction,cross_selective_scan,CrossScan_Ab_2direction,SelectiveScanFake,SelectiveScanMamba,CrossScan_Ab_1direction,CrossMerge_Ab_2direction
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input, x_size):
        ...
        # x_kan = self.ln_2(x)                        # [B,H,W,C]
        # x_kan = x_kan.view(B, H * W, C)             # [B,N,C] where N = H*W
        # x_kan = self.conv_blk(x_kan, H, W)           # [B,N,C]
        # x_kan = x_kan.view(B, H, W, C).contiguous() # back to [B,H,W,C]

        # x = x * self.skip_scale2 + x_kan
