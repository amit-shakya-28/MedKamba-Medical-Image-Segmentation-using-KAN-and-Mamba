...
def parse_args():
    ...
    parser.add_argument('--output_dir', default='outputs_busi_MIDL_seed44', help='output dir')
...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        if config['deep_supervision']:
            ...
            iou_, dice_, recall_,hd95_,hd_, specificity_, precision_ = indicators(outputs[-1], target)
            
        else:
            output = model(input)
            loss = criterion(output, target)
            iou, dice, _ = iou_score(output, target)
            iou_, dice_,recall_,hd95_,hd_, specificity_, precision_ = indicators(output, target)
...
def main():
    ...
    base='/content/drive/MyDrive/Akanksha/MIDL_2/U-KAN/Seg_UKAN/DATA/split_seed_44'
    train_img_dir = os.path.join(base, 'train', 'images')
    # print(train_img_dir)
    # exit(0)
    train_msk_dir = os.path.join(base,'train', 'masks')
    val_img_dir   = os.path.join(base,'val',   'images')
    val_msk_dir   = os.path.join(base,'val',   'masks')
    # print(train_img_dir)
    # print("Exists?", os.path.exists(train_img_dir))
    # Data loading code
    # print("Files inside:", os.listdir(train_img_dir))
    img_ids = sorted(glob(os.path.join(train_img_dir, '*' + img_ext)))
...
#Added this to resume the training.    
    start_epoch = 0
    best_iou = 0.0
    best_dice = 0.0
    if config['resume'] is not None:
        ...
        if os.path.exists(checkpoint_path):
            ...
            # fallback: it's just a raw model.pth
              model.load_state_dict(checkpoint)
              print("Only model weights loaded (no optimizer/scheduler/epoch info)")
              start_epoch = 0  
        else:
    ...
    for epoch in range(start_epoch, config['epochs']):
        ...
        # AFTER you computed val_log and stepped scheduler
        trigger += 1

        # If this epoch is better -> update bests first
        if val_log['iou'] > best_iou:
            best_iou = val_log['iou']
            best_dice = val_log['dice']

            # Save model weights only
            torch.save(model.state_dict(), f'{output_dir}/{exp_name}/model_updated.pth')

            # Save full checkpoint with UPDATED bests and log
            torch.save({
                'epoch': epoch,
                ...
                'best_dice': best_dice,
                'log': log
            }, f'{output_dir}/{exp_name}/checkpoint.pth')
